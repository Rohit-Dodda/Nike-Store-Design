...
import { selectCartItems, selectCartState, selectTotalAmount, selectTotalQuantity, setClearCartItems, setCloseCart, setGetTotals } from "../app/CartSlice.js";
  ...
  return (
      ...
      >      <div className={`blur-effect-theme h-screen max-w-xl w-full absolute right-0`}>
        ...
        {cartItems?.length === 0 ? <CartEmpty onCartToggle={onCartToggle} /> : <div>
          <div className="flex items-start justify-start flex-col gap-y-7 lg:gap-y-5 overflow-y-scroll scroll-smooth h-[81vh] scroll-hidden py-3">
            {cartItems?.map((val, i) => (
              <CartItem key={i} val={val} />
            ))}
          </div>
